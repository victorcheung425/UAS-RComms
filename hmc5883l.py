...
class HMC5883L(object):
    ...
    def __init__(self, bus, address, name, samples=3, rate=4, gain=1, sampling_mode=0, x_offset=-110, y_offset=-370, z_offset=-90):
        self.bus = bus
    ...
    def read_bearing(self):
        ...
        if bearing < 0:
            return (bearing + (HMC5883L.TWO_PI))*180/math.pi
        else:
    ...
    def read_compensated_bearing(self, pitch, roll):
        ...
        if bearing < 0:
            return (bearing + (HMC5883L.TWO_PI))*180/pi
        else:
